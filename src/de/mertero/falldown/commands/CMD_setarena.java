				...
				if(cmd.getName().equalsIgnoreCase("setarena")){
					if(CloudAPI.getPlayerAPI().getRank(p).equals(Rank.ADMIN)) {
						if(args.length >= 2 || args.length == 0){
							p.sendMessage(Main.getInstance().getPrefix() + "§7/setarena <Nummer>");
							return true;
						}
						
						try {
							int number =  Integer.parseInt(args[0]);
							Main.getInstance().getLm().setArenaSpawn(number, p.getLocation());
							p.sendMessage(Main.getInstance().getPrefix() + "Du hast den Arenaspawn Nummer §6["+number+"] §7gesetzt");
